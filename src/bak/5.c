...
#define FRAME_INTERVAL_NS (1000000000 / 30) // 15 FPS for stability (66.67 ms)
...
void set_wallpaper(Display *display, Window root, GC gc, int screen, int mon_x, int mon_y, int mon_width, int mon_height,
        ...
        if (width <= 0 || height <= 0 || mon_width <= 0 || mon_height <= 0) {
                fprintf(stderr, "Invalid dimensions: width=%d, height=%d, mon_width=%d, mon_height=%d\n",
                        width, height, mon_width, mon_height);
                return;
        }

        // Validate RGB data
        if (!rgb_data) {
                fprintf(stderr, "RGB data is NULL\n");
                return;
        }

        // Create Imlib2 image from raw RGB data
        Imlib_Image image = imlib_create_image_using_data(width, height, (uint32_t *)rgb_data/*(DATA32 *)rgb_data*/);
        if (!image) {
                fprintf(stderr, "Failed to create Imlib2 image (width=%d, height=%d)\n", width, height);
                return;
        }
        ...
        int screen_depth = DefaultDepth(display, screen);
        Pixmap pixmap = XCreatePixmap(display, root, screen_width, screen_height, screen_depth);
        if (!pixmap) {
        ...
        if (root_attrs.depth != screen_depth) {
                fprintf(stderr, "Depth mismatch: pixmap=%d, screen=%d\n", screen_depth, root_attrs.depth);
                XFreePixmap(display, pixmap);
                imlib_free_image();
                return;
        }
...
int main(int argc, char **argv) {
        ...
        printf("Monitor dimensions: width=%d, height=%d, x=%d, y=%d\n", mon_width, mon_height, mon_x, mon_y);
        ...
        printf("Video stream found: width=%d, height=%d, codec_id=%d\n",
        ...
        // Initialize scaling context to match monitor resolution
        struct SwsContext *sws_ctx = sws_getContext(codec_ctx->width, codec_ctx->height, codec_ctx->pix_fmt,
                                                    mon_width, mon_height, AV_PIX_FMT_RGB24,
        ...
        int rgb_buffer_size = av_image_get_buffer_size(AV_PIX_FMT_RGB24, mon_width, mon_height, 1);
        ...
        if (av_image_fill_arrays(rgb_frame->data, rgb_frame->linesize, rgb_buffer, AV_PIX_FMT_RGB24,
                                 mon_width, mon_height, 1) < 0) {
                fprintf(stderr, "Could not fill RGB frame arrays\n");
                av_free(rgb_buffer);
                av_frame_free(&frame);
                av_frame_free(&rgb_frame);
                sws_freeContext(sws_ctx);
        ...
        // Calculate frame interval for 15 FPS
        double fps = av_q2d(fmt_ctx->streams[video_stream_idx]->r_frame_rate);
        printf("Video FPS: %.2f, Target FPS: 15\n", fps);
        int64_t frame_interval = (int64_t)(AV_TIME_BASE / 15.0); // Target 15 FPS
        int64_t next_pts = 0;
                        ...
                        if (packet->stream_index == video_stream_idx) {
                                if (packet->pts >= next_pts) {
                                        ...
                                        while (ret == AVERROR(EAGAIN)) {
                                                // Decoder needs more frames to be read
                                                ...
                                                if (ret == 0) {
                                                        ...
                                                        set_wallpaper(display, root, gc, screen, mon_x, mon_y, mon_width, mon_height,
                                                                      rgb_frame->data[0], mon_width, mon_height);
                                                        // Update next presentation time
